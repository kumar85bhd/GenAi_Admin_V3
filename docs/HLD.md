...
- **Authentication**: Managed via a FastAPI backend using JWT (JSON Web Tokens) with HS256 signature verification.
  - **Single Sign-On (SSO)**: The architecture supports integration with external Identity Providers (IdPs) via OAuth 2.0 / OIDC. When configured, the backend handles the OAuth flow and issues an internal JWT for session management.
